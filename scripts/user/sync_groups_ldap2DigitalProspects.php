...
/**
 * \file scripts/user/sync_groups_ldap2DigitalProspects.php
 * \ingroup ldap member
 * \brief Script to update groups into DigitalProspects from LDAP
 */
...
print "----- To DigitalProspects database:\n";
...
if (empty($conf->global->LDAP_GROUP_DN)) {
	print $langs->trans("Error").': '.$langs->trans("LDAP setup for groups not defined inside DigitalProspects");
	exit(-1);
}
...
if ($result >= 0) {
	...
	// We disable synchro DigitalProspects-LDAP
	...
	if (is_array($ldaprecords)) {
		...
		foreach ($ldaprecords as $key => $ldapgroup) {
			...
			// 1 - Association des utilisateurs du groupe LDAP au groupe DigitalProspects
			$userList = array();
			$userIdList = array();
			...
			// 2 - Suppression des utilisateurs du groupe DigitalProspects qui ne sont plus dans le groupe LDAP
