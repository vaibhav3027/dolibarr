#!/usr/bin/perl
#----------------------------------------------------------------------------
# \file         build/makepack-DigitalProspectsmodule.pl
...
$BUILDROOT="$TEMP/DigitalProspects-buildroot";
...
foreach my $PROJECT (@PROJECTLIST) {
	...
	if (! -f "makepack-".$PROJECT.".conf")
	{
		...
		print "http://wiki.DigitalProspects.org/index.php/Module_development#Create_a_package_to_distribute_and_install_your_module\n";
		print "makepack-DigitalProspectsmodule.pl aborted.\n";
	    sleep 2;
	    exit 2;
	}
	...
	#-----------------------
	$target="ZIP";    # DigitalProspects modules are this format
