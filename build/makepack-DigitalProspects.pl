#!/usr/bin/perl
#----------------------------------------------------------------------------
# \file         build/makepack-DigitalProspects.pl
# \brief        DigitalProspects package builder (tgz, zip, rpm, deb, exe, aps)
# \author       (c)2004-2020 Laurent Destailleur  <eldy@users.sourceforge.net>
#
# This is list of constant you can set to have generated packages moved into a specific dir: 
#DESTIBETARC='/media/HDDATA1_LD/Mes Sites/Web/DigitalProspects/DigitalProspects.org/files/lastbuild'
#DESTISTABLE='/media/HDDATA1_LD/Mes Sites/Web/DigitalProspects/DigitalProspects.org/files/stable'
...
$PROJECT="DigitalProspects";
$PUBLISHSTABLE="eldy,DigitalProspects\@frs.sourceforge.net:/home/frs/project/DigitalProspects";
$PUBLISHBETARC="DigitalProspects\@vmprod1.DigitalProspects.org:/home/DigitalProspects/asso.DigitalProspects.org/DigitalProspects_documents/website/www.DigitalProspects.org/files";
...
if (! $ENV{"DESTIBETARC"} || ! $ENV{"DESTISTABLE"})
{
	...
	print "Example: DESTIBETARC='/media/HDDATA1_LD/Mes Sites/Web/DigitalProspects/DigitalProspects.org/files/lastbuild'\n";
	print "Example: DESTISTABLE='/media/HDDATA1_LD/Mes Sites/Web/DigitalProspects/DigitalProspects.org/files/stable'\n";
	print "$PROG.$Extension aborted.\n";
	sleep 2;
	exit 1;
}
...
if ($nboftargetok) {
	...
	if ($nbofpublishneedchangelog)
	{
		...
		{
			print 'cd ~/git/DigitalProspects_'.$MAJOR.'.'.$MINOR.'; git log `git rev-list --boundary '.$MAJOR.'.'.$MINOR.'..origin/develop | grep ^- | cut -c2- | head -n 1`.. --no-merges --pretty=short --oneline | sed -e "s/^[0-9a-z]* //" | grep -e \'^FIX\|NEW\|CLOSE\' | sort -u | sed \'s/FIXED:/FIX:/g\' | sed \'s/FIXED :/FIX:/g\' | sed \'s/FIX :/FIX:/g\' | sed \'s/FIX /FIX: /g\' | sed \'s/CLOSE/NEW/g\' | sed \'s/NEW :/NEW:/g\' | sed \'s/NEW /NEW: /g\' > /tmp/aaa';
		}
		else			# For a maintenance release
		{
			#print 'cd ~/git/DigitalProspects_'.$MAJOR.'.'.$MINOR.'; git log '.$MAJOR.'.'.$MINOR.'.'.($BUILD-1).'.. --no-merges --pretty=short --oneline | sed -e "s/^[0-9a-z]* //" | grep -e \'^FIX\|NEW\' | sort -u | sed \'s/FIXED:/FIX:/g\' | sed \'s/FIXED :/FIX:/g\' | sed \'s/FIX :/FIX:/g\' | sed \'s/FIX /FIX: /g\' | sed \'s/CLOSE/NEW/g\'| sed \'s/NEW :/NEW:/g\' | sed \'s/NEW /NEW: /g\' > /tmp/aaa';
			print 'cd ~/git/DigitalProspects_'.$MAJOR.'.'.$MINOR.'; git log '.$MAJOR.'.'.$MINOR.'.'.($BUILD-1).'.. | grep -v "Merge branch" | grep -v "Merge pull" | grep "^ " | sed -e "s/^[0-9a-z]* *//" | grep -e \'^FIX\|NEW\|CLOSE\' | sort -u | sed \'s/FIXED:/FIX:/g\' | sed \'s/FIXED :/FIX:/g\' | sed \'s/FIX :/FIX:/g\' | sed \'s/FIX /FIX: /g\' | sed \'s/CLOSE/NEW/g\' | sed \'s/NEW :/NEW:/g\' | sed \'s/NEW /NEW: /g\' > /tmp/aaa';
		}
		print "\n";
		if (! $ret)
		{
	...
	if ($nboftargetneedbuildroot)
	{
		...
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/DigitalProspects_*.deb`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/DigitalProspects_*.dsc`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/DigitalProspects_*.tar.gz`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/DigitalProspects_*.tar.xz`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/DigitalProspects-*.deb`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/DigitalProspects-*.rpm`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/DigitalProspects-*.tar`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/DigitalProspects-*.tar.gz`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/DigitalProspects-*.tar.xz`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/DigitalProspects-*.tgz`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/DigitalProspects-*.xz`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/DigitalProspects-*.zip`;
		...
		$ret=`rm -f  $BUILDROOT/$PROJECT/dev/DigitalProspects_changes.txt`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/dev/README`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/DigitalProspects_screenshot2.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/DigitalProspects_screenshot3.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/DigitalProspects_screenshot4.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/DigitalProspects_screenshot5.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/DigitalProspects_screenshot6.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/DigitalProspects_screenshot7.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/DigitalProspects_screenshot8.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/DigitalProspects_screenshot9.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/DigitalProspects_screenshot10.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/DigitalProspects_screenshot11.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/DigitalProspects_screenshot12.png`;
	...
	foreach my $target (sort keys %CHOOSEDTARGET) 
	{
		...
		if ($target =~ /RPM/)	                 # Linux only 
		{
			...
			$ret=`cp "$SOURCE/build/rpm/DigitalProspects-forrpm.patch" "$RPMDIR/SOURCES"`;
			$ret=`chmod 644 $RPMDIR/SOURCES/DigitalProspects-forrpm.patch`;
		...
		if ($target eq 'DEB') 
		{
			...
			$ret=`rm -f  $BUILDROOT/$PROJECT.tmp/build/debian/DigitalProspects.config`;
			$ret=`rm -f  $BUILDROOT/$PROJECT.tmp/build/debian/DigitalProspects.desktop`;
			$ret=`rm -f  $BUILDROOT/$PROJECT.tmp/build/debian/DigitalProspects.docs`;
			$ret=`rm -f  $BUILDROOT/$PROJECT.tmp/build/debian/DigitalProspects.install`;
			$ret=`rm -f  $BUILDROOT/$PROJECT.tmp/build/debian/DigitalProspects.lintian-overrides`;
			$ret=`rm -f  $BUILDROOT/$PROJECT.tmp/build/debian/DigitalProspects.postrm`;
			$ret=`rm -f  $BUILDROOT/$PROJECT.tmp/build/debian/DigitalProspects.postinst`;
			$ret=`rm -f  $BUILDROOT/$PROJECT.tmp/build/debian/DigitalProspects.templates`;
			$ret=`rm -f  $BUILDROOT/$PROJECT.tmp/build/debian/DigitalProspects.templates.futur`;
			...
			$ret=`cp -f  "$SOURCE/build/debian/DigitalProspects.desktop"        	"$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret=`cp -f  "$SOURCE/build/debian/DigitalProspects.docs"        		"$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret=`cp -f  "$SOURCE/build/debian/DigitalProspects.install" 	        "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret=`cp -f  "$SOURCE/build/debian/DigitalProspects.lintian-overrides"  "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret=`cp -f  "$SOURCE/build/debian/DigitalProspects.xpm"  		      	"$BUILDROOT/$PROJECT.tmp/debian"`;
			...
			$ret=`cp -f  "$SOURCE/build/debian/DigitalProspects.config"         "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret=`cp -f  "$SOURCE/build/debian/DigitalProspects.postinst"       "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret=`cp -f  "$SOURCE/build/debian/DigitalProspects.postrm"         "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret=`cp -f  "$SOURCE/build/debian/DigitalProspects.templates"      "$BUILDROOT/$PROJECT.tmp/debian"`;
	...
	foreach my $target (sort keys %CHOOSEDPUBLISH) 
	{
		...
		%filestoscansf=(
			...
			"$DESTI/package_rpm_generic/$FILENAMERPM"=>'DigitalProspects installer for Fedora-Redhat-Mandriva-Opensuse (DoliRpm)',
			"$DESTI/package_rpm_generic/$FILENAMERPMSRC"=>'none',						# none means it won't be published on SF
			"$DESTI/package_debian-ubuntu/${FILENAMEDEB}_all.deb"=>'DigitalProspects installer for Debian-Ubuntu (DoliDeb)',
			...
			"$DESTI/package_windows/$FILENAMEEXEDOLIWAMP.exe"=>'DigitalProspects installer for Windows (DoliWamp)',
			"$DESTI/standard/$FILENAMETGZ.tgz"=>'DigitalProspects ERP-CRM',
			"$DESTI/standard/$FILENAMETGZ.zip"=>'DigitalProspects ERP-CRM'
		);
		...
		if ($target eq 'ASSO' && $BUILD =~ /[a-z]/i)   { 	# Not stable
			%filestoscansf=(
				"$DESTI/$FILENAMERPM"=>'DigitalProspects installer for Fedora-Redhat-Mandriva-Opensuse (DoliRpm)',
				"$DESTI/${FILENAMEDEB}_all.deb"=>'DigitalProspects installer for Debian-Ubuntu (DoliDeb)',
				"$DESTI/$FILENAMEEXEDOLIWAMP.exe"=>'DigitalProspects installer for Windows (DoliWamp)',
				"$DESTI/$FILENAMETGZ.tgz"=>'DigitalProspects ERP-CRM',
				"$DESTI/$FILENAMETGZ.zip"=>'DigitalProspects ERP-CRM'
			);
			%filestoscanstableasso=(
				"$DESTI/$FILENAMERPM"=>'',
		...
		if ($target eq 'SF' || $target eq 'ASSO') 
		{
			...
			foreach my $file (sort keys %filestoscan)
			{
				...
				#$command="ssh eldy,DigitalProspects\@frs.sourceforge.net mkdir -p \"$destFolder\"";
				#print "$command\n";	
				#my $ret=`$command 2>&1`;
