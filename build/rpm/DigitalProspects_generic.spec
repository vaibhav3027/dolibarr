...
%if 0%{?fedora} || 0%{?rhel_version} || 0%{?centos_version}
Source0: https://www.DigitalProspects.org/files/lastbuild/package_rpm_redhat-fedora/%{name}-%{version}.tgz
%else
%if 0%{?mdkversion}
Source0: https://www.DigitalProspects.org/files/lastbuild/package_rpm_mandriva/%{name}-%{version}.tgz
%else
%if 0%{?suse_version}
Source0: https://www.DigitalProspects.org/files/lastbuild/package_rpm_opensuse/%{name}-%{version}.tgz
%else
Source0: https://www.DigitalProspects.org/files/lastbuild/package_rpm_generic/%{name}-%{version}.tgz
%endif
%endif
%endif
...
%else%_datadir/DigitalProspects/htdocs/datapolicy
%if 0%{?suse_version}
...
%{__mkdir} $RPM_BUILD_ROOT%_datadir/DigitalProspects/htdocs/datapolicy
...
#Commented as it fails with error: /usr/bin/install: cannot stat build/rpm/DigitalProspects.desktop: No such file or directory
...
%suse_update_desktop_file DigitalProspects Office Finance
...
echo Run post script of packager DigitalProspects_generic.spec
...
%if 0%{?fedora} || 0%{?rhel_version} || 0%{?centos_version} || 0%{?mdkversion}
export apachelink="%{_sysconfdir}/httpd/conf.d/DigitalProspects.conf"
...
%else
export installconfig="%{_sysconfdir}/DigitalProspects/install.forced.php"
...
%else
# Create install.forced.php into DigitalProspects install directory
...
then
  ...
  %{__cat} /usr/share/DigitalProspects/build/rpm/install.forced.php.generic | sed -e 's/__SUPERUSERLOGIN__/'$superuserlogin'/g' | sed -e 's/__SUPERUSERPASSWORD__/'$superuserpassword'/g' > $installconfig
...
%if 0%{?fedora} || 0%{?rhel_version} || 0%{?centos_version}
  grep -q -c "DigitalProspects_font_DOL_DEFAULT_TTF_BOLD" $config || echo "<?php \$DigitalProspects_font_DOL_DEFAULT_TTF_BOLD='/usr/share/fonts/dejavu/DejaVuSans-Bold.ttf'; ?>" >> $config
%else
%if 0%{?mdkversion}
  grep -q -c "DigitalProspects_font_DOL_DEFAULT_TTF_BOLD" $config || echo "<?php \$DigitalProspects_font_DOL_DEFAULT_TTF_BOLD='/usr/share/fonts/TTF/dejavu/DejaVuSans-Bold.ttf'; ?>" >> $config
%else
%if 0%{?suse_version}
...
%else
  grep -q -c "DigitalProspects_font_DOL_DEFAULT_TTF_BOLD" $config || echo "<?php \$DigitalProspects_font_DOL_DEFAULT_TTF_BOLD='/usr/share/fonts/truetype/ttf-dejavu/DejaVuSans-Bold.ttf'; ?>" >> $config
%endif
%endif
%endif
fi
...
%else
  echo Add SE Linux permissions for DigitalProspects
  # semanage add records into /etc/selinux/targeted/contexts/files/file_contexts.local
  semanage fcontext -a -t httpd_sys_rw_content_t "/etc/DigitalProspects(/.*)?"
  semanage fcontext -a -t httpd_sys_rw_content_t "/var/lib/DigitalProspects(/.*)?"
  restorecon -R -v /etc/DigitalProspects
  restorecon -R -v /var/lib/DigitalProspects
%endif
%if 0%{?fedora} || 0%{?rhel_version} || 0%{?centos_version} || 0%{?mdkversion} || 0%{?suse_version}
%else
fi
