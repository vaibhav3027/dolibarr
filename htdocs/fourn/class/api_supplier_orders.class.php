 ...
 */
class SupplierOrders extends DigitalProspectsApi
{
	/**
	 *
	...
	{
		if (!DigitalProspectsApiAccess::$user->rights->fournisseur->commande->lire) {
		...
		if (!DigitalProspectsApi::_checkAccessToResource('fournisseur', $this->order->id, '', 'commande')) {
	...
	{
		if (!DigitalProspectsApiAccess::$user->rights->fournisseur->commande->creer) {
		...
		if ($this->order->create(DigitalProspectsApiAccess::$user) < 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->order->update($id, DigitalProspectsApiAccess::$user))
			return $this->get($id);
	...
	{
		if (!DigitalProspectsApiAccess::$user->rights->fournisseur->commande->supprimer) {
			throw new RestException(401);
		}
		$result = $this->order->fetch($id);
		if (!$result) {
		...
		if ($this->order->delete(DigitalProspectsApiAccess::$user) < 0) {
	...
	public function validate($id, $idwarehouse = 0, $notrigger = 0)
	{
		...
		$result = $this->order->valid(DigitalProspectsApiAccess::$user, $idwarehouse, $notrigger);
		if ($result == 0) {
