...
define('USEDigitalProspectsEDITOR', 1);
...
$pathofwebsite = $DigitalProspects_main_data_root.'/website/'.$websitekey;
...
{
	DigitalProspects_set_const($db, 'WEBSITE_EDITINLINE', 1);
	setEventMessages($langs->trans("FeatureNotYetAvailable"), null, 'warnings');
	DigitalProspects_set_const($db, 'WEBSITE_SUBCONTAINERSINLINE', 0); // Force disable of 'Include dynamic content'
	header("Location: ".$_SERVER["PHP_SELF"].'?website='.GETPOST('website', 'alphanohtml').'&pageid='.GETPOST('pageid', 'int'));
	exit;
}
if ($action == 'unseteditinline')
{
	DigitalProspects_del_const($db, 'WEBSITE_EDITINLINE');
	header("Location: ".$_SERVER["PHP_SELF"].'?website='.GETPOST('website', 'alphanohtml').'&pageid='.GETPOST('pageid', 'int'));
	exit;
}
if ($action == 'setshowsubcontainers')
{
	DigitalProspects_set_const($db, 'WEBSITE_SUBCONTAINERSINLINE', 1);
	DigitalProspects_set_const($db, 'WEBSITE_EDITINLINE', 0); // Force disable of edit inline
	header("Location: ".$_SERVER["PHP_SELF"].'?website='.GETPOST('website', 'alphanohtml').'&pageid='.GETPOST('pageid', 'int'));
	exit;
}
if ($action == 'unsetshowsubcontainers')
{
	DigitalProspects_del_const($db, 'WEBSITE_SUBCONTAINERSINLINE');
	header("Location: ".$_SERVER["PHP_SELF"].'?website='.GETPOST('website', 'alphanohtml').'&pageid='.GETPOST('pageid', 'int'));
	exit;
}
...
if ($action == 'addcontainer')
{
	...
	if (GETPOSTISSET('fetchexternalurl'))
	{
		...
		if (!$error)
		{
			...
			else
			{
				...
				// Normalize links href to DigitalProspects internal naming
	...
	if (!$error)
	{
		...
		{
			$robotcontent = "# Robot file. Generated with DigitalProspects\nUser-agent: *\nAllow: /public/\nDisallow: /administrator/";
		...
		{
			$readmecontent = "Website generated by DigitalProspects ERP CRM";
...
if ($action == 'updatecss')
{
	...
	else
	{
		...
		if (!$error)
		{
    	    ...
    	    $htmlheadercontent.= "if (! defined('USEDigitalProspectsSERVER') && ! defined('USEDigitalProspectsEDITOR')) { require_once './master.inc.php'; } // Load env if not already loaded"."\n";
    		...
    		$csscontent .= "if (! defined('USEDigitalProspectsSERVER') && ! defined('USEDigitalProspectsEDITOR')) { require_once __DIR__.'/master.inc.php'; } // Load env if not already loaded\n"; // For the css, we need to set path of master using the dirname of css file.
    		...
    		$jscontent .= "if (! defined('USEDigitalProspectsSERVER') && ! defined('USEDigitalProspectsEDITOR')) { require_once __DIR__.'/master.inc.php'; } // Load env if not already loaded\n"; // For the css, we need to set path of master using the dirname of css file.
    	    ...
    	    $robotcontent.= "if (! defined('USEDigitalProspectsSERVER') && ! defined('USEDigitalProspectsEDITOR')) { require_once './master.inc.php'; } // Load env if not already loaded"."\n";
    		...
    		if (!$result)
    		{
       		...
       		$manifestjsoncontent .= "if (! defined('USEDigitalProspectsSERVER') && ! defined('USEDigitalProspectsEDITOR')) { require_once __DIR__.'/master.inc.php'; } // Load env if not already loaded\n"; // For the css, we need to set path of master using the dirname of css file.
       		...
       		$readmecontent.= "if (! defined('USEDigitalProspectsSERVER') && ! defined('USEDigitalProspectsEDITOR')) { require_once __DIR__.'/master.inc.php'; } // Load env if not already loaded"."\n";	// For the css, we need to set path of master using the dirname of css file.
	...
	if ($action == 'confirm_createpagefromclone')
	{
		...
		if (!$error)
		{
			...
			if ($newwebsiteid > 0 && $newwebsiteid != $object->id)
			{
				...
				$pathofwebsitenew = $DigitalProspects_main_data_root.'/website/'.$tmpwebsite->ref;
			}
			else
			{
				$tmpwebsite = $object;
		...
		if (!empty($_FILES) || GETPOSTISSET('templateuserfile'))
		{
			...
			if (!$error)
			{
				...
				else
				{
					...
					DigitalProspects_set_const($db, 'WEBSITE_SUBCONTAINERSINLINE', 1, 'chaine', 0, '', $conf->entity);
...
if (!GETPOST('hide_websitemenu'))
{
	...
	if ($websitekey && $websitekey != '-1' && !in_array($action, array('editcss', 'editmenu', 'importsite', 'file_manager', 'replacesite', 'replacesiteconfirm')) && !$file_manager)
	{
		...
		if (($pageid > 0 && $atleastonepage) && ($action == 'preview' || $action == 'createfromclone' || $action == 'createpagefromclone' || $action == 'deletesite'))
		{
			...
			$htmltext = $langs->trans("PreviewSiteServedByDigitalProspects", $langs->transnoentitiesnoconv("Page"), $langs->transnoentitiesnoconv("Page"), $realpage, $dataroot);
			$htmltext .= '<br>'.$langs->trans("CheckVirtualHostPerms", $langs->transnoentitiesnoconv("ReadPerm"), DOL_DOCUMENT_ROOT);
			$htmltext .= '<br>'.$langs->trans("CheckVirtualHostPerms", $langs->transnoentitiesnoconv("WritePerm"), DOL_DATA_ROOT.'/website<br>'.DOL_DATA_ROOT.'/medias');
		...
		{
			$url = 'https://wiki.DigitalProspects.org/index.php/Module_Website';
...
if ($action == 'preview' || $action == 'createfromclone' || $action == 'createpagefromclone')
{
	if ($pageid > 0 && $atleastonepage)
	{
		...
		// Ouput page under the DigitalProspects top menu
		...
		*/
		...
		// Clean style that may affect global style of DigitalProspects
