 ...
 */
class Boms extends DigitalProspectsApi
{
    /**
    ...
    {
		if (!DigitalProspectsApiAccess::$user->rights->bom->read) {
        ...
        if (!DigitalProspectsApi::_checkAccessToResource('bom', $this->bom->id, 'bom_bom')) {
    ...
    {
        if (!DigitalProspectsApiAccess::$user->rights->bom->write) {
            throw new RestException(401);
        }
        ...
        if (!$this->bom->create(DigitalProspectsApiAccess::$user)) {
    ...
    public function put($id, $request_data = null)
    {
        ...
        if ($this->bom->update($id, DigitalProspectsApiAccess::$user) > 0)
        {
            return $this->get($id);
        }
        else
    ...
    {
    	if (!DigitalProspectsApiAccess::$user->rights->bom->delete) {
			throw new RestException(401);
		}
        $result = $this->bom->fetch($id);
        if (!$result) {
		...
		if (!$this->bom->delete(DigitalProspectsApiAccess::$user))
        {
