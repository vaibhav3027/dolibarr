...
/**
 *		\file 		htdocs/admin/tools/DigitalProspects_import.php
 *		\ingroup	core
...
<?php
print $langs->trans("RestoreDesc3", $DigitalProspects_main_db_name).'<br><br>';
?>

<?php print $langs->trans("DatabaseName").' : <b>'.$DigitalProspects_main_db_name.'</b>'; ?><br><br>
...
if (in_array($type, array('mysql', 'mysqli')))
{
	...
	$param = $DigitalProspects_main_db_name;
	$param .= " -h ".$DigitalProspects_main_db_host;
	...
	if (!empty($DigitalProspects_main_db_pass))
	{
		...
		$paramclear .= " -p".$DigitalProspects_main_db_pass;
	}
    ...
    if (empty($_GET["showpass"]) && $DigitalProspects_main_db_pass) {
...
elseif (in_array($type, array('pgsql')))
{
    ...
    $param = " -d ".$DigitalProspects_main_db_name;
    $param .= " -h ".$DigitalProspects_main_db_host;
    if (!empty($DigitalProspects_main_db_port)) $param .= " -p ".$DigitalProspects_main_db_port;
    $param .= " -U ".$DigitalProspects_main_db_user;
    $paramcrypted = $param;
    ...
    print ajax_autoselect('restorecommand');
    //if (empty($_GET["showpass"]) && $DigitalProspects_main_db_pass) print '<br><a href="'.$_SERVER["PHP_SELF"].'?showpass=1&amp;radio_dump=postgresql_options">'.$langs->trans("UnHidePassword").'</a>';
    //else print '<br><a href="'.$_SERVER["PHP_SELF"].'?showpass=0&amp;radio_dump=mysql_options">'.$langs->trans("HidePassword").'</a>';
    print '</div>';
    print '</fieldset>';
}
