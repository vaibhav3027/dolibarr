...
if (!defined('DONOTLOADCONF') && file_exists($conffile) && filesize($conffile) > 8) // Test on filesize is to ensure that conf file is more that an empty template with just <?php in first line
{
	...
	if ($result)
	{
		if (empty($DigitalProspects_main_db_type)) $DigitalProspects_main_db_type = 'mysql'; // For backward compatibility
		...
		// Clean parameters
		$DigitalProspects_main_data_root        = isset($DigitalProspects_main_data_root) ?trim($DigitalProspects_main_data_root) : '';
...
define('DOL_DATA_ROOT', (isset($DigitalProspects_main_data_root) ? $DigitalProspects_main_data_root : ''));
...
if (empty($DigitalProspects_main_db_character_set)) $DigitalProspects_main_db_character_set = ($conf->db->type == 'mysql' ? 'latin1' : ''); // Old installation
$conf->db->character_set = $DigitalProspects_main_db_character_set;
if (empty($DigitalProspects_main_db_collation)) $DigitalProspects_main_db_collation = ($conf->db->type == 'mysql' ? 'latin1_swedish_ci' : ''); // Old installation
...
function pHeader($soutitre, $next, $action = 'none')
{
	...
	print '<meta name="keywords" content="help, center, DigitalProspects, doliwamp">'."\n";
	print '<meta name="description" content="DigitalProspects help center">'."\n";
	print '<link rel="stylesheet" type="text/css" href="default.css">'."\n";
	print '<title>'.$langs->trans("DigitalProspectsHelpCenter").'</title>'."\n";
	print '</head>'."\n";
