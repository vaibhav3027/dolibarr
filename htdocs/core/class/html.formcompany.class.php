	...
	public function showRoles($htmlname, Contact $contact, $rendermode = 'view', $selected = array())
	{
		if ($rendermode === 'view') {
			...
			foreach ($contact->roles as $key => $val) {
				$toprint[] = '<li class="select2-search-choice-DigitalProspects noborderoncategories" style="background: #aaa;">'.$val['label'].'</li>';
			}
			return '<div class="select2-container-multi-DigitalProspects" style="width: 90%;" id="'.$htmlname.'"><ul class="select2-choices-DigitalProspects">'.implode(' ', $toprint).'</ul></div>';
		}
