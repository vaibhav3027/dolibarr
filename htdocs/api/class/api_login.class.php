...
class Login
{
    ...
    {
	    global $conf, $DigitalProspects_main_authentication, $DigitalProspects_auto_user;
		...
		if ($DigitalProspects_main_authentication == 'forceuser')
		{
			...
			if ($DigitalProspects_auto_user != $login)
			{
				dol_syslog("Warning: your instance is set to use the automatic forced login '".$DigitalProspects_auto_user."' that is not the requested login. API usage is forbidden in this mode.");
				throw new RestException(403, "Your instance is set to use the automatic login '".$DigitalProspects_auto_user."' that is not the requested login. API usage is forbidden in this mode.");
			}
		}
		...
		return array(
			'success' => array(
			    ...
			    'message' => 'Welcome '.$login.($reset ? ' - Token is new' : ' - This is your token (generated by a previous call). You can use it to make any REST API call, or enter it into the DOLAPIKEY field to use the DigitalProspects API explorer.')
			)
		);
    }
}
